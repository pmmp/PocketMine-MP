...
## Internals
- `PrepareEncryptionTask` now checks the validity of the provided client public key in the constructor. This ensures earlier error reporting when used incorrectly. Previously it would crash during the `AsyncTask` where the source of the error couldn't be traced. (@dktapps)
