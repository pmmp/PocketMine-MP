...
## Custom PHP binaries
Because FrogCore requires several non-standard PHP extensions and configuration, PMMP provides scripts to build custom binaries for running FrogCore, as well as prebuilt binaries.
...
1. `git clone https://github.com/MrHoller/FrogCore.git`
2. `composer install`
...
## Building `FrogCore.phar`
Run `composer make-server` using your preferred PHP binary. It'll drop a `FrogCore.phar` into the current working directory.

You can also use the `--out` option to change the output filename.

There is a bug in PHP that might cause an error which looks like this:
```
Fatal error: Uncaught BadMethodCallException: unable to create temporary file in FrogCore/build/server-phar.php:119
```
...
## Running FrogCore from source code
