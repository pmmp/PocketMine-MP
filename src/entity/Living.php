		...
		if($activeEffectsTag !== null){
			foreach($activeEffectsTag as $e){
				...
				$this->effectManager->add(new EffectInstance(
					$effect,
					$e->getInt(self::TAG_EFFECT_DURATION) === -1 ? Limits::INT32_MAX : $e->getInt(self::TAG_EFFECT_DURATION),
		...
		if(count($this->effectManager->all()) > 0){
			...
			foreach($this->effectManager->all() as $effect){
				$effects[] = CompoundTag::create()
					...
					->setInt(self::TAG_EFFECT_DURATION, $effect->getDuration() === Limits::INT32_MAX ? -1 : $effect->getDuration())
