...
class Bamboo extends Transparent{
	...
	public function onInteract(Item $item, int $face, Vector3 $clickVector, ?Player $player = null) : bool{
		if($item instanceof Fertilizer){
			$top = $this->seekToTop();
			if($top->grow(self::getMaxHeight($top->position->getFloorX(), $top->position->getFloorZ()), mt_rand(1, 2), $item, $player)){
				$item->pop();
				return true;
			}
		}elseif($item instanceof ItemBamboo){
			if($this->seekToTop()->grow(PHP_INT_MAX, 1, $item, $player)){
				$item->pop();
				return true;
			}
		}
	...
	private function grow(int $maxHeight, int $growAmount, ?Item $item, ?Player $player) : bool{
		$world = $this->position->getWorld();
		...
		$ev = new StructureGrowEvent($this, $tx, $item, $player);
		$ev->call();
		if($ev->isCancelled()){
			return false;
		}
	...
	public function onRandomTick() : void{
		...
		if($this->ready){
			$this->ready = false;
			if($world->getFullLight($this->position) < 9 || !$this->grow(self::getMaxHeight($this->position->getFloorX(), $this->position->getFloorZ()), 1, null, null)){
