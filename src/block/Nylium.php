		...
		for($x = -2; $x <= 2; ++$x){
			for($z = -2; $z <= 2; ++$z){
				if($random->nextBoundedInt(3) === 0){
					...
					// Resolve factory to a Block instance. Factories can be:
					// - a callable returning a Block
					// - a string naming a VanillaBlocks accessor (e.g. "CRIMSON_ROOTS")
					// - a Block instance
					if(is_callable($factory)){
						$place = $factory();
					}elseif(is_string($factory)){
						// Resolve using the registry accessors at runtime (these will be registered by then)
						$place = VanillaBlocks::{$factory}();
					}elseif($factory instanceof Block){
						$place = $factory;
					}else{
						// Unknown factory type, skip
						continue;
					}

					// Use a fresh clone for placement checks
					$place = clone $place;
