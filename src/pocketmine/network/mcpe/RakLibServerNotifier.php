...
/**
 * Wakes up the server from sleep when a packet is received on a RakLibInterface.
 */
class RakLibServerNotifier extends SleeperNotifier implements IRakLibNotifier{
	...
	public function onServerNotify(Server $server) : void{
		foreach($server->getNetwork()->getInterfaces() as $interface){
			if($interface instanceof RakLibInterface and $interface->getSleeper() === $this){
				$server->getNetwork()->processInterface($interface);
				break;
			}
		}
	}
