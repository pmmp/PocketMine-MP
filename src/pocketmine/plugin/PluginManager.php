...
class PluginManager{
	 ...
	 */
	public function registerLoader(PluginLoader $loader) : void{
		$this->pluginLoaders[get_class($loader)] = $loader;
	}
		...
		foreach($loaders ?? $this->pluginLoaders as $loader){
			if($loader->canLoadPlugin($path)){
				...
				if($manifestLoader !== null){
					...
					$description = $manifestLoader->getPluginDescription($prefixed);
					if($description instanceof PluginDescription){
						...
						$manifestLoader->registerPlugin($prefixed, $this->server->getLoader());
	...
	public function loadPlugins(string $directory, array $newLoaders = null){
		...
		if(is_array($newLoaders)){
			...
			foreach($newLoaders as $key){
				if(isset($this->pluginLoaders[$key])){
					$loaders[$key] = $this->pluginLoaders[$key];
				}
			}
		}else{
			$loaders = $this->pluginLoaders;
		}
		foreach($loaders as $loader){
			foreach(new \DirectoryIterator($directory) as $file){
				...
				try{
					$description = $manifestLoader->getPluginDescription($loader->getAccessProtocol() . $file);
