...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	public function spawnTo(Player $player){
		if($this->spawned and $player->spawned and $this->isAlive() and $player->getLevel() === $this->level and $player->canSee($this) and !$this->isSpectator()){
	...
	public function isAlive() : bool{
		return parent::isAlive() && !$this->isSpectator();
	}
	...
	public function handleDataPacket(DataPacket $packet){
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::MOVE_PLAYER_PACKET:
				...
				$revert = false;
				if(!$this->isAlive() and !$this->isSpectator() or !$this->spawned){
					$revert = true;
			...
			case ProtocolInfo::ANIMATE_PACKET:
				if(!$this->isAlive() and !$this->isSpectator() or $this->spawned === false){
					break;
				}
			...
			case ProtocolInfo::CONTAINER_CLOSE_PACKET:
				if(!$this->isAlive() or !$this->isSpectator() and $this->spawned === false or $packet->windowid === 0){
					break;
				}
				$this->craftingType = 0;
