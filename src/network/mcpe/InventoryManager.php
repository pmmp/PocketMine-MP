	...
	public function syncSlot(Inventory $inventory, int $slot) : void{
		...
		if($windowId !== null){
			...
			if($clientSideItem === null or !$clientSideItem->equalsExact($currentItem)){
				$this->session->sendDataPacket(InventorySlotPacket::create($windowId, $slot, ItemStackWrapper::legacy(TypeConverter::getInstance()->coreItemStackToNet($this->session->getProtocolId(), $currentItem))));
			}
